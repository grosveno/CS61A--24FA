...
def buy(fruits_to_buy, prices, total_amount):
    ...
    [9 apples][1 kiwi]
    """
    def add(fruits, amount, cart):       # fruits为需要买的水果，amount为剩下的钱，cart为当前的订单
        if fruits == [] and amount == 0: # 成功后，打印；否则不打印
            print(cart)
        elif fruits and amount > 0:
            fruit = fruits[0]        # 当前买的水果
            price = prices[fruit]    # 水果的单价
            for k in range(1, amount // price + 1):     # 买的数量在[1, amount // price]之间
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    city_dest = make_city('Dest', lat, lon)
    if distance(city_dest, city_a) > distance(city_dest, city_b):
