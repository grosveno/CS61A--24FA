...
def about(subject):
    ...
    def check(paragraph):
        ...
        for word in word_list:
            word = lower(remove_punctuation(word))
            if word in subject:
                return True
        return False
    return check
...
def accuracy(typed, source):
    ...
    if typed_len == 0 and source_len == 0:
        ...
        count, iter_len = 0, min(typed_len, source_len)
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    count, minute = len(typed) / 5, elapsed / 60
    return count / minute
    # END PROBLEM 4
...
def final_diff(typed, source, limit):
    ...
    I just use minimum_mewtations function, don't add more function.
    """
    if limit < 0:
        return 1
    if typed == source:
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[player[i] - player[i - 1] for i in range(len(player)) if i > 0] for player in tpp]  
    # END PROBLEM 9
